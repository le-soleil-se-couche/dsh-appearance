 ...
 * truth for the UI plugin client bundles in this repository (previously copied per-package
...
/** Externals resolved from the official host's platform module table. */
...
function clientConfig(id: string, entry: string): UserConfig {
  // Keep physical CSS paths out of virtual module ids. Rolldown may surface
  // virtual ids in generated region comments, so embedding an absolute path
  // would leak the builder's local checkout into the published bundle.
  const cssVirtualFiles = new Map<string, string>()
  ...
  return {
    ...
    plugins: [{
      ...
      resolveId(source: string, importer: string | undefined) {
        ...
        const stablePath = relative(REPOSITORY_ROOT, abs).split(sep).join('/')
        const virtualId = CSS_VIRTUAL_PREFIX + stablePath + CSS_VIRTUAL_SUFFIX
        cssVirtualFiles.set(virtualId, abs)
        return virtualId
      },
      async load(virtualId: string) {
        if (!virtualId.startsWith(CSS_VIRTUAL_PREFIX)) return null
        const fileId = cssVirtualFiles.get(virtualId)
        if (fileId === undefined) return null
        // The virtual id otherwise hides the physical stylesheet from Rolldown's watch graph.
        this.addWatchFile(fileId)
        const source = await readFile(fileId)
        const { code, exports: cssExports } = transform({
          // CSS class hashes must remain identical in another clean checkout.
          filename: relative(REPOSITORY_ROOT, fileId).split(sep).join('/'),
          code: source,
