...
import {
  createDshSkinRunner,
  DSH_SKIN_REPO_FALLBACK,
  makeSkinCenterRoutes,
  resolveDshSkinInvocation,
  SKIN_CENTER_API_PREFIX,
  type DshSkinCommand,
} from '../src/routes.ts'
...
describe('skin-center routes', () => {
  ...
  it('falls back to the repo-local script only for PATH ENOENT on POSIX', async () => {
    ...
    expect(calls).toEqual([
      ...
      { file: '/repo/scripts/dsh-skin', args: ['use', 'qq98'] },
    ])
  })

  it('reports the exact fallback path when neither executable exists', async () => {
    const command: DshSkinCommand = async () => {
      throw Object.assign(new Error('spawn ENOENT'), { code: 'ENOENT' })
    }
    const run = createDshSkinRunner(command, '/repo/scripts/dsh-skin')
    await expect(run(['current'])).rejects.toThrow(
      'dsh-skin CLI not found on PATH and repo fallback is unavailable at /repo/scripts/dsh-skin',
    )
    expect(basename(DSH_SKIN_REPO_FALLBACK)).toBe('dsh-skin')
  })

  it('runs the repo fallback through Node and uses a shell only for Windows PATH shims', () => {
    expect(resolveDshSkinInvocation('/repo/scripts/dsh-skin', ['current'], '/repo/scripts/dsh-skin', 'win32')).toEqual({
      file: process.execPath,
      args: ['/repo/scripts/dsh-skin', 'current'],
      shell: false,
    })
    expect(resolveDshSkinInvocation('dsh-skin', ['current'], '/repo/scripts/dsh-skin', 'win32')).toEqual({
      file: 'dsh-skin',
      args: ['current'],
      shell: true,
    })
  ...
  it('prefers the checkout fallback before PATH on Windows', async () => {
    const calls: Array<{ file: string; args: string[] }> = []
    const command: DshSkinCommand = async (file, args) => {
      calls.push({ file, args })
      return 'claude-code\n'
    }
    const run = createDshSkinRunner(command, 'C:\\repo\\scripts\\dsh-skin', 'win32')
    await expect(run(['current'])).resolves.toBe('claude-code\n')
    expect(calls).toEqual([{ file: 'C:\\repo\\scripts\\dsh-skin', args: ['current'] }])
  })
  ...
  it('GET /fonts returns the host-enumerated local family list', async () => {
    const { run } = stubRunner([])
    const server = await serve(makeSkinCenterRoutes({
      run,
      fontFamilies: async () => ['PingFang SC', 'SF Mono'],
    }))
    const response = await call(server.port, 'GET', `${SKIN_CENTER_API_PREFIX}/fonts`)
    await server.close()
    expect(response.status).toBe(200)
    expect(response.body).toEqual({ ok: true, families: ['PingFang SC', 'SF Mono'] })
  })
  ...
  it('GET /bundle/<id> serves a real skin client bundle as JavaScript', async () => {
    const { run } = stubRunner([])
    const server = await serve(makeSkinCenterRoutes({ run }))
    const response = await call(server.port, 'GET', `${SKIN_CENTER_API_PREFIX}/bundle/claude-code`)
    await server.close()
    expect(response.status).toBe(200)
